...
//        this.setLocationRelativeTo(null);
        ...
        new Thread(() -> {
            ...
            switch (this.algorithm) {
                ...
                case Definitions.ALGORITHM_DFS:
                    result = dfsAlgorithm(startRow, startColumn);
                    break;
    ...
    public boolean dfsAlgorithm(int row, int col) {

        if (!visited[row][col]) {
            setSquareAsVisited(row, col, true);

            if (row == values.length - 1 && col == values.length - 1) {
                return true;
            }

            if (col < values.length - 1) {
                if (unvisitedNeighbour(row, col + 1))
                    return true;
            }

            if (row < values.length - 1) {
                if (unvisitedNeighbour(row + 1, col)) {
                    return true;
                }
            }
            if (col > 0) {
            ...
            setSquareAsVisited(row, col, false);
    ...
    private boolean unvisitedNeighbour(int xLocation, int yLocation) {
        if (!(visited[xLocation][yLocation])) {
            if (values[xLocation][yLocation] == Definitions.EMPTY) {
                return dfsAlgorithm(xLocation, yLocation);
            }
        }
        return false;
    }
